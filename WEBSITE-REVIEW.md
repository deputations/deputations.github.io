...
- **Data pipeline:**
  ...
  - Legacy fallback backend: ~~`apps-script/*.gs` (Google Apps Script)~~ — **retired 2026-07-30 (P3-5).** Folder deleted; `window.DEPUTATIONS_API` removed; contact + report-vacancy fallback branches removed. The FAQ "report a discrepancy" button is intentionally disabled (its backend was the same Apps Script).
